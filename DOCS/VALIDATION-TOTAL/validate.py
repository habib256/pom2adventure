...
with tempfile.TemporaryDirectory(prefix='total-validate-') as work:
    ...
    try:
        ...
        key(b'\t'); select(0, 'TOTAL'); key(b'C'); wait(lambda: value('ops') == 15, 'copy dir', 30); p.stable(); shot('dir-copy')
        ...
        key(b'\t'); select(0, 'BANC'); key(b'C'); wait(lambda: value('ops') == 18, 'copy tree depth 2', 30); p.stable()
        ...
        ok('D supprime /SCOSWAMP/BANC et ses deux niveaux', not any(r[40:].startswith('BANC ') for r in rows()) and value('ops') == 22)
        key(b'\t'); select(0, 'BANC'); key(b'D'); wait_text('Delete BANC and everything inside?'); key(b'Y'); p.stable()
        ok('D supprime DHGR/BANC et tout son contenu', not any(r[:38].startswith('BANC ') for r in rows()) and value('ops') == 26)
