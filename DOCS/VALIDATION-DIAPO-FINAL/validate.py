...
with tempfile.TemporaryDirectory(prefix='diapo-validate-') as work:
 ...
 try:
  ...
  time.sleep(.05)  # let the subsequent video-switch instructions complete
  code=pathlib.Path('SCOSWAMP/DIAPO.CODE.BIN').read_bytes();off=3072+labels['_get_current_mode']-0x4000
  assert code[off:off+3]==bytes.fromhex('a2 00 ad')
  mode_addr=int.from_bytes(code[off+3:off+5],'little')
  assert p.peek(mode_addr,1)==b'\x01'
  time.sleep(1)
  assert p.peek(mode_addr,1)==b'\x01'
