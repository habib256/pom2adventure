...
with tempfile.TemporaryDirectory(prefix='total-validate-') as work:
    ...
    try:
        ...
        ok('le format reconnu est annonce', has('N000.RLE: DHGR RLE (DHRR v1), 16384 bytes on screen.'))
        ...
        key(b'\t'); select(0, 'TOTAL'); key(b'C'); wait(lambda: value('ops') == 17, 'copy dir', 30); p.stable(); shot('dir-copy')
        ok('C copie le dossier TOTAL entier (mkdir + 4 fichiers, aide et TOTAL.CFG compris)', has('4 files copied') and any(r[40:].startswith('TOTAL ') and '<DIR>' in r[40:] for r in rows()))
        ...
        key(b'\t'); select(0, 'BANC'); key(b'C'); wait(lambda: value('ops') == 22, 'copy tree depth 2', 30); p.stable()
        ok('C copie un arbre a deux niveaux (BANC/TOTAL/4 fichiers)', has('4 files copied') and any(r[40:].startswith('BANC ') for r in rows()))
        ...
        ok('D supprime /SCOSWAMP/BANC et ses deux niveaux', not any(r[40:].startswith('BANC ') for r in rows()) and value('ops') == 28)
        key(b'\t'); select(0, 'BANC'); key(b'D'); wait_text('Delete BANC and everything inside?'); key(b'Y'); p.stable()
        ok('D supprime DHGR/BANC et tout son contenu', not any(r[:38].startswith('BANC ') for r in rows()) and value('ops') == 34)
        ...
        ok('X sauve et revient : NOTE, TXT, 11 octets', any(r[40:].startswith('NOTE ') and 'TXT' in r[40:] and '  11 ' in r[40:] for r in rows()) and value('ops') == 35)
        ...
        ok('Entree sur un .MB le joue sur la Mockingboard', has('Playing WELCOME.MB on the Mockingboard in slot') and value('playing', 1) == 1)
        mb = pathlib.Path('SCOSWAMP/MUSIC/WELCOME.MB.BIN').read_bytes()
        ok('le flux MB1 est monte en AUX $1000', p.peek(0x1000, len(mb), 'aux') == mb)
        key(b'P'); p.stable()
